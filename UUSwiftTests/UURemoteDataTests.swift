...
class UURemoteDataTests: XCTestCase
{
   ...
   // private static let testUrl : String = "https://publicdomainarchive.com/wp-content/uploads/2014/10/splitshire-70.jpg"
...
//        }
    ...
    private func fetchMultipleBadUrl(count: Int) -> [String]
    {
        ...
        for td in testData
        {
            let exp = UUExpectationForMethod(tag: td)
            expectations.append(exp)
        }
        
        for td in testData
        {
            let request = UUHttpRequest(url: td)
            ...
            { (response: UUHttpResponse) in
                ...
                //{
                    ...
                    {
                        let exp = expectations.remove(at: 0)
                        exp.fulfill()
                    }
                //}
